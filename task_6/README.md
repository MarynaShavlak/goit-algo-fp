...
рюкзак 0/1, де кожну страву беремо щонайбільше раз.
...
їх, поки вистачає бюджету; якщо страва не влізла — пропускає її й пробує наступні
(`continue`, а не `break`), тож устигає добрати дешевші.
...
`b` беремо краще з двох: `dp[i-1][b]` (без страви) або `dp[i-1][b-cost] +
calories` (зі стравою).
...
відновлюємо зворотним проходом по таблиці: страву `i` взято, якщо `dp[i][b]`
кращий за `dp[i-1][b]`. За це платимо пам'яттю O(n · budget) на 2D-таблицю; якщо
потрібна лише максимальна калорійність (без переліку страв), вистачило б одного
рядка й O(budget). Для фіксованого меню й помірного бюджету ДП — правильний
вибір; жадібний доречний, коли потрібна миттєва відповідь і похибка прийнятна.
